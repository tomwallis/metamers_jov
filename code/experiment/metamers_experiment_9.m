% Function to run metamers experiment 8.
...
function metamers_experiment_9()
%% Set random seed:
seed = sum(100*clock);
...
filepathToHere=pwd;
gen_file = fullfile(filepathToHere, '../stimuli/generation_params_exp_9.yaml');
...
data_path = fullfile(filepathToHere, '../../raw-data/experiment-9/');
im_path = fullfile(filepathToHere, '../../stimuli/experiment-9/final_ims/');
...
if strcmp(surround, 'y')
    param_ok = 0;
    while param_ok == 0
        surround_source = input('Surround "nat" or "synth"?:', 's');
        % check parameters:
        if strcmp(surround_source, 'nat')
            param_ok = 1;
        elseif strcmp(surround_source, 'synth')
            param_ok = 1;
        else
            warning('Not a valid option. Try again')
        end
...
datafilename = fullfile(data_path, strcat('experiment-9_sub_',subj,'_session_', num2str(session), '.csv'));
...
eyedata_fname = fullfile(eye_data_path, strcat('experiment-9_sub_',subj,'_session_', num2str(session), '.edf'));
...
surround_source = repmat(surround_source, length(cond), 1);
...
data = table(cond, surround_cond, surround_source, ...
...
try
    ...
    for trial=1:n_trials
        ...
        surround_source = table2array(data(this_trial_idx, 'surround_source'));
        ...
        if strcmp(surround_cond, 'surround')
            inner_fname = get_inner_fname(im_code, surround_source);
            inner_fname = char(fullfile(im_path, inner_fname));
            outer_fname = get_outer_fname(im_code, surround_source);
...
%% Some subfunctions
    ...
    function fname = get_image_fname(im_code)
        fname = sprintf('%s_mid_natural.png',im_code);
    end


    function fname = get_synth_fname(im_code, synth_num)
        fname = sprintf('%s_mid_synth_%1d.png',im_code, synth_num);
    end

    function fname = get_inner_fname(im_code, surround_source)
        fname = sprintf('%s_inner_%s.png',im_code, surround_source);
    end

    function fname = get_outer_fname(im_code, surround_source)
        fname = sprintf('%s_outer_%s.png',im_code, surround_source);
    end
