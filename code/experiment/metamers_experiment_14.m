% Function to run metamers experiment 14: blur discrimination in oddity, with smaller
% stimulus levels compared to experiment 13 and only one patch size.
...
function metamers_experiment_14()
%% Set random seed:
seed = sum(100*clock);
...
filepathToHere=pwd;
gen_file = fullfile(filepathToHere, '../stimuli/generation_params_exp_14.yaml');
...
data_path = fullfile(filepathToHere, '../../raw-data/experiment-14/');
im_path = fullfile(filepathToHere, '../../stimuli/experiment-14/final_ims/');
...
param_ok = 0;
while param_ok == 0
    blur_sigma = input('Which blur level to do? [0.4, 0.6, 0.8, 1., 2., 4.]:');
    % check parameters:
...
datafilename = fullfile(data_path, strcat('experiment-14_sub_',subj,'_session_', num2str(session), '.csv'));
...
eyedata_fname = fullfile(eye_data_path, strcat('experiment-14_sub_',subj,'_session_', num2str(session), '.edf'));
