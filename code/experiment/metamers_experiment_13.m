% Function to run metamers experiment 13: blur discrimination in oddity.
...
function metamers_experiment_13()
%% Set random seed:
seed = sum(100*clock);
...
filepathToHere=pwd;
gen_file = fullfile(filepathToHere, '../stimuli/generation_params_exp_13.yaml');
...
data_path = fullfile(filepathToHere, '../../raw-data/experiment-13/');
im_path = fullfile(filepathToHere, '../../stimuli/experiment-13/final_ims/');
...
param_ok = 0;
while param_ok == 0
    patch_size = input('Which patch size to do? [32, 256]:');
    % check parameters:
    if ~any([gen_params.sizes{(1:end)}] == patch_size)
        warning('requested patch size is not a valid option. Try again.');
    else
        param_ok = 1;
    end
    [~, size_idx] = max([gen_params.sizes{(1:end)}] == patch_size);
end
...
param_ok = 0;
while param_ok == 0
    blur_sigma = input('Which blur level to do? [0.5, 1., 2., 3., 5., 8., 12., 16.]:');
    % check parameters:
    if ~any([gen_params.blur_sigmas{(1:end)}] == blur_sigma)
        warning('requested patch size is not a valid option. Try again.');
    else
        param_ok = 1;
    end
...
datafilename = fullfile(data_path, strcat('experiment-13_sub_',subj,'_session_', num2str(session), '.csv'));
...
eyedata_fname = fullfile(eye_data_path, strcat('experiment-13_sub_',subj,'_session_', num2str(session), '.edf'));
...
im_code = table2array(source_dat(source_dat.size==patch_size, 'filename'));
...
target_loc = target_vec;
target_loc = num2str(target_loc);

% the surround conditions:
surround_cond = repmat(surround_cond, length(im_code), 1);
...
data = table(surround_cond, ...
             oddball, im_code, target_loc, ...
             jitter_1, jitter_2, jitter_3);

n_trials = height(data); % number of trials

% compute stimulus size for these conditions:
stim_size = (patch_size / gen_params.pix_per_deg);
...
for trial=1:n_trials
    ...
    dat_struct(trial).size = stim_size;
    dat_struct(trial).blur_sigma = blur_sigma;
...
try
    ...
    for trial=1:n_trials
        ...
        blur_sigma = table2array(data(this_trial_idx, 'blur_sigma'));
        ...
        im_0_fname = get_mid_natural(im_code);  % the natural image
        im_0_fname = char(fullfile(im_path, im_0_fname));
        im_1_fname = get_mid_blurred(im_code, blur_sigma);  % blurred image
        ...
        if strcmp(surround_cond, 'surround')
            inner_fname = get_inner_fname(im_code);
            inner_fname = char(fullfile(im_path, inner_fname));
            outer_fname = get_outer_fname(im_code);
            ...
            inner_radius = gen_params.inner_patch_px{(size_idx)};
            outer_radius = gen_params.outer_patch_px{(size_idx)};

            % determine drawing rects:
            inner_rect_base = [0, 0, gen_params.inner, gen_params.inner];
            outer_rect_base = [0, 0, gen_params.outer, gen_params.outer];
...
fprintf('The experiment took %2.1f minutes', experiment_duration);
...
%% Some subfunctions
    ...
    function fname = get_mid_natural(im_code)
       fname =  sprintf('%s_mid_nat.png',im_code);
    end


    function fname = get_mid_blurred(im_code, blur_sigma)
       fname =  sprintf('%s_mid_blur_%0.1f.png',im_code, blur_sigma);
    end


    function fname = get_inner_fname(im_code)
       fname =  sprintf('%s_inner_nat.png',im_code);
    end


    function fname = get_outer_fname(im_code)
       fname =  sprintf('%s_outer_nat.png',im_code);
    end
