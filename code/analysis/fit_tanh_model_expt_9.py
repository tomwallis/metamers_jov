...
if experiment_num == 9:
    dat = helpers.experiment_9_data_ready_for_analysis(dat)
else:
    raise ValueError('specify a data munging function!')

# create binomial groupings
dat = helpers.binomial_binning(dat, ['subj', 'cond', 'surround', 'patch_size'],
                               rule_of_succession=rule_of_succession)

# design matrix ignores subject:
X = patsy.dmatrix('~ C(cond, Sum) * C(surround, Sum)', dat)

dat = subj_to_numeric(dat)

# grid for model predictions:
s_pred = np.linspace(0, 13, num=99)
pred_dat = pu.misc.expand_grid({'s_pred': s_pred,
                                'cond': np.unique(dat['cond']),
                                'surround': np.unique(dat['surround']),
                                'subj': np.unique(dat['subj'])})
pred_dat = subj_to_numeric(pred_dat)
pred_X = patsy.dmatrix('~ C(cond, Sum) * C(surround, Sum)', pred_dat)
pred_ss = pred_dat['subj_num']

# grid for calculating model params:
param_dat = pu.misc.expand_grid({'cond': np.unique(dat['cond']),
                                 'surround': np.unique(dat['surround']),
                                 'subj': np.unique(dat['subj'])})
param_dat = subj_to_numeric(param_dat)
param_X = patsy.dmatrix('~ C(cond, Sum) * C(surround, Sum)', param_dat)
param_ss = param_dat['subj_num']

# grid for calculating population params:
pop_param_dat = pu.misc.expand_grid({'cond': np.unique(dat['cond']),
                                     'surround': np.unique(dat['surround'])})
pop_param_X = patsy.dmatrix('~ C(cond, Sum) * C(surround, Sum)', pop_param_dat)
...
pred_frame.rename(columns={'s_pred': 'patch_size'}, inplace=True)
